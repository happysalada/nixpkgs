    ...
    systemd.services =
      {
        "prefect-server" = {
          ...
          serviceConfig = {
            ...
            Environment =
              let
                sqliteUrl = "sqlite+aiosqlite://$PREFECT_HOME/prefect.db";
      ...
      // lib.concatMapAttrs (poolName: poolCfg: {
        ...
        "prefect-worker-${poolName}" = {
          ...
          serviceConfig = {
            DynamicUser = true;
            RuntimeDirectory = "prefect-worker-${poolName}";
            WorkingDirectory = "/run/prefect-worker-${poolName}";
            ...
            ExecPreStart =
              let
                sqliteUrl = "sqlite+aiosqlite:///run/prefect-worker-${poolName}/prefect.db";
                setIfNotNull = name: val: ''
                  if [ -n "${val}" ]; then
                    ${pkgs.prefect}/bin/prefect config set ${name}="${val}"
                  fi
                '';
              in
              [
                "${pkgs.prefect}/bin/prefect config unset PREFECT_API_DATABASE_CONNECTION_URL || true"
                ''
                  if [ "${cfg.database}" = "sqlite" ]; then
                    ${pkgs.prefect}/bin/prefect config set PREFECT_API_DATABASE_CONNECTION_URL=${sqliteUrl};
                  else
                    ${pkgs.prefect}/bin/prefect config set PREFECT_API_DATABASE_CONNECTION_URL=postgresql+asyncpg://${cfg.databaseUser}:$DBPASSWORD@${cfg.databaseHost}:${cfg.databasePort}/${cfg.databaseName};
                  fi
                ''
                "${pkgs.prefect}/bin/prefect config unset PREFECT_UI_API_URL || true"
                "${pkgs.prefect}/bin/prefect config unset PREFECT_UI_SERVE_BASE || true"
                "${pkgs.prefect}/bin/prefect config unset PREFECT_UI_URL || true"
                "${setIfNotNull "PREFECT_UI_API_URL" cfg.uiApiUrl}"
                "${setIfNotNull "PREFECT_UI_SERVE_BASE" cfg.uiServeBase}"
                "${setIfNotNull "PREFECT_UI_URL" cfg.uiUrl}"
              ];
